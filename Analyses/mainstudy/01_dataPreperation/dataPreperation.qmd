...
#> scales
tmp_numeric <- str_subset(string = colnames(dat), pattern = "^EcologicalDimension|^Bioinspiration|^evalInf|^GAToRS|Design$|Topic$")
...
```
### video: Subjective Outcome Evaluation Scale

```{r}
psych::cor.plot(r = cor(questionnaire[, str_subset(string = colnames(questionnaire), pattern = "^evalInf")], use =  "pairwise.complete.obs"))
```
### video: Situational Interest Scale

for topic:

```{r}
psych::cor.plot(r = cor(questionnaire[, str_subset(string = colnames(questionnaire), pattern = "Topic$")], use =  "pairwise.complete.obs"))

questionnaire$boringTopic <- 8 - questionnaire$boringTopic
questionnaire$unnecessaryTopic <- 8 - questionnaire$unnecessaryTopic
questionnaire$unimportantTopic <- 8 - questionnaire$unimportantTopic
...
```{r}
psych::cor.plot(r = cor(questionnaire[, str_subset(string = colnames(questionnaire), pattern = "Design$")], use =  "pairwise.complete.obs"))

questionnaire$boringDesign <- 8 - questionnaire$boringDesign
questionnaire$unnecessaryDesign <- 8 - questionnaire$unnecessaryDesign
questionnaire$unimportantDesign <- 8 - questionnaire$unimportantDesign
...
write_rds(
  x = df_brainstorming,
  file = "df_brainstorming.rds"    # Save R object for later use
)
```

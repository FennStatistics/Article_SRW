...
// >>> Randomly assign framing condition with equal probability (50/50)
var framingCondition = Math.random() < 0.5 ? "tech-inspired" : "bio-inspired";
...
// >>> Randomly assign survey order with equal probability (50/50)
var ecologicalFirst = Math.random() < 0.5;
// Set both variables based on random outcome
var orderSurveys_Ecological_First = ecologicalFirst;
var orderSurveys_Bioinspiration_First = !ecologicalFirst;
console.log("Ecological Survey First:", orderSurveys_Ecological_First);
console.log("Bioinspiration Survey First:", orderSurveys_Bioinspiration_First);
  ...
  messageHandlers: {
    ...
    commit: () => {
      ...
      if (typeof jatos.jQuery === "function") {
        ...
        // check if framing condition is provided via URL parameter framingCondition
        const allowedConditions = ["tech-inspired", "bio-inspired"];
        if (
          URLparams_global.framingCondition &&
          allowedConditions.includes(URLparams_global.framingCondition)
        ) {
          framingCondition = URLparams_global.framingCondition;
        }

        console.log("framingCondition =", framingCondition);
        study.options.datastore.set("framingCondition", framingCondition);

        console.log("condExpert =", condExpert);
        study.options.datastore.set("condExpert", condExpert);

        console.log(
          "Order surveys shown - orderSurveys_Ecological_First:",
          orderSurveys_Ecological_First,
          "orderSurveys_Bioinspiration_First:",
          orderSurveys_Bioinspiration_First
        );
        study.options.datastore.set(
          "orderSurveys_Ecological_First",
...
let player;
// Target: 7 minutes 8 seconds = 428 seconds (currently 12 sec for testing)
const REQUIRED_WATCH_TIME_SECONDS = 420; // test: 15 seconds

let watchTimeAccumulated = 0;

const VideoPage_htmlForm = new lab.html.Form({
  title: "Videopage",
  content: textObj.waitingPage,
  messageHandlers: {
    run: function anonymous() {
      if (framingCondition === "tech-inspired") {
        $("#manipulation1_text").html("technical principles");
      } else {
        $("#manipulation1_text").html("biological principles");
      }

      // Set video URL depending on framing condition
      const techUrl = "https://www.youtube.com/embed/7EC8t8trG-A";
      const bioUrl = "https://www.youtube.com/embed/0hVrMzJ4lOE";

      const baseUrl = framingCondition === "tech-inspired" ? techUrl : bioUrl;

      const videoParams =
        "?controls=0&modestbranding=0&rel=0&fs=1&disablekb=1&playsinline=1&enablejsapi=1";

      $("#study-video").attr("src", baseUrl + videoParams);

      const continueButton = document.getElementById("continue");
      // $('#continue').prop('disabled', true);
      // Falls aus irgendeinem Grund nicht disabled, hier nochmal zur Sicherheit:
      if (continueButton) {
        continueButton.disabled = true;
      }

      // Start function after iframe was loaded
      onYouTubeIframeAPIReady();

      // Total time the video has actually been watched (in seconds)
      // ID of the interval timer that accumulates watch time
      let watchInterval = null;
      // Timestamp of the last tick (ms) for precise timing
      let lastTickTime = null;

      // Diese Funktion wird von der YouTube IFrame API aufgerufen
      function onYouTubeIframeAPIReady() {
        player = new YT.Player("study-video", {
          events: {
      ...
      function startWatchTimer() {
        if (watchInterval !== null) return; // bereits aktiv
        ...
        watchInterval = setInterval(() => {
          ...
          watchTimeAccumulated += deltaSeconds;

          // Optional: console.log für Debugging
          console.log("Watch time accumulated:", watchTimeAccumulated);
          $("#watched-time").text(Math.floor(watchTimeAccumulated));

          if (watchTimeAccumulated >= REQUIRED_WATCH_TIME_SECONDS) {
      ...
      function onPlayerStateChange(event) {
        ...
        // PAUSED, ENDED, BUFFERING, CUED, UNSTARTED
        else {
          stopWatchTimer();
        }

        // Falls das Video zu Ende läuft und die Zeit etwas knapp war,
        // hier optional nochmal sicherstellen, dass der Button freigeschaltet wird,
        // wenn z.B. das Video sehr nah an 7:08 ist.
        if (event.data === YT.PlayerState.ENDED) {
          if (
            watchTimeAccumulated >= REQUIRED_WATCH_TIME_SECONDS &&
            continueButton
          ) {
            continueButton.disabled = false;
          }
      ...
      // --- Stop video on defocus (tab/window loses focus) ---
      // When the page becomes hidden (user switches tab or minimises window),
      // pause the video and stop the watch timer.
    ...
    commit: () => {
      study.options.datastore.set("watch_time_video", watchTimeAccumulated);
      // progress bar
      numElementsCounter++;
      document.querySelector(".progress-bar").style.width =
        (numElementsCounter / numElements) * 100 + "%";
  ...
  items: [
    {
      ...
      label:
        "Answer the following statements and indicate to what extent you agree with them.",
  ...
  messageHandlers: {
    run: function anonymous() {
      ...
      $('[id="techname"]').each(function () {
        $(this).text("Soft Robot Walker");
      });

      // add id to button
      $('button[type="submit"]').attr("id", "continue");
  ...
  items: [
    {
      ...
      label:
        "Please read each statement carefully and indicate how much you agree with it.",
      help: "Please answer each statement, even if you're not entirely sure what your answer should be.",
      shuffle: false,
      name: "Bioinspiration",
    },
...
const Sequence_Ecological_Bioinspiration = new lab.flow.Sequence({
  ...
  shuffle: false,
  content: [EcologicalDimension_Scale_htmlForm, Bioinspiration_Scale_htmlForm],
});
...
const Sequence_Bioinspiration_Ecological = new lab.flow.Sequence({
  ...
  shuffle: false,
  content: [Bioinspiration_Scale_htmlForm, EcologicalDimension_Scale_htmlForm],
});
...
const SustainabilityReflection_htmlForm = new lab.html.Form({
  ...
  messageHandlers: {
    run: () => {
      const pillars = [
        {
          key: "ecological",
          title: "Ecological aspect of sustainability",
          ...
          placeholder: "Please write how the Soft Robot  Walker may relate to ecological sustainability..."
        },
        {
          key: "social",
          title: "Social aspect of sustainability",
          ...
          placeholder: "Please write how the Soft Robot  Walker may relate to social sustainability..."
        },
        {
          key: "economic",
          title: "Economic aspect of sustainability",
          ...
          placeholder: "Please write how the Soft Robot  Walker may relate to economic sustainability..."
        },
      ];
  ...
  placeholder="${p.placeholder}" required></textarea>
...
// >>> aaa
const EvaluateInformation_1_htmlForm = new lab.html.Page({
  title: "EvaluateInformation_1",
  items: [
    {
      required: Required_Testing,
      type: "likert",
...
// >>> bbb
let slidersInteracted = 0;

const EvaluateInformation_2_htmlForm = new lab.html.Form({
  title: "EvaluateInformation_2",
  content: textObj.EvaluateInformation,
  messageHandlers: {
    run: () => {
...
const Evaluation_sequence = new lab.flow.Sequence({
  title: "Evaluation Sequence",
  shuffle: true,
  content: [
    EvaluateInformation_1_htmlForm,
    EvaluateInformation_2_htmlForm,
  ],
});
        ...
        if (
          ...
          if (condExpert === true) {
            ...
            jatos.endStudyAndRedirect(
              "https://app.prolific.com/submissions/complete?cc=C21HCJSF",
              true,
...
const study = new lab.flow.Sequence({
  metadata: {
    title:
      "Framing Bio-Inspiration: Communication Strategies and Public Perception of Sustainable Robotics",
    description:
      "This project empirically tests when and how the biomimetic promise shows up in lay judgments.",
    repository: "https://github.com/FennStatistics/Article_SRW",
    contributors:
      "Julius Fenn, Wilhelm Gros, Stephanie Bugler, Philipp Höfele, Falk Tauber, Andrea Kiesel",
  },
  plugins: [
  ...
  content: [
    ...
    // >>> video with two conditions ("tech-inspired", "bio-inspired"), randomly assigned
    VideoPage_htmlForm,

    // >>> survey scales with randomized order
    // main DVs:
    Sequence_Ecological_Bioinspiration,
    Sequence_Bioinspiration_Ecological,

    // secondary DV: reflection on sustainability:
    SustainabilityReflection_htmlForm,

    // >>> evaluate presented information
    TransitionToEvaluation_htmlForm,
    Evaluation_sequence,


    // >>> brain storm application cases
    ...
    GAToRS_sequence, // survey scale - include?
